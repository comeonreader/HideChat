...
* Key：`user:profile:{userUid}`
* TTL：10 分钟（后续按活跃度动态调整）
* Payload：`userUid`、`nickname`、`avatarUrl`、`email`
* 提供者：UserService，在读取/更新资料时自动写入
* 消费方：
  * User 模块 `GET /api/user/me` 直接命中
  * Contact / Conversation 列表批量拉取用户昵称、头像，避免重复 join
  * 后续消息推送、会话列表刷新可引用相同缓存结构

## 21.2 预留扩展方向

1. **会话摘要缓存**（下一阶段 Message 模块使用）
   * Key：`conversation:summary:{conversationId}`
   * 保存最近一条消息的 preview、类型、时间戳，供会话列表快速渲染
2. **文件上传短期态**（下一阶段 File 模块使用）
   * Key：`file:upload:{fileId}`
   * 缓存上传签名、加密标记、过期时间，保障回调幂等

目前的用户资料缓存已经验证 Redis 读写链路，RedisTemplate、序列化、key 规范均稳定，后续模块按上述 key 继续扩展即可，无需新增基础设施。
