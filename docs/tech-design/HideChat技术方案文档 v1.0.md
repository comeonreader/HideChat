...
* `luckyNumber`：用户在伪装页输入的数字，用于运势页展示，非 `luckyCode`

数据库落点：

* 新建 `im_disguise_lucky_code` 表
* 字段至少包含 `code_value / status / effective_start_at / effective_end_at / created_at / updated_at`

失败策略：

* 不展示错误提示
* 保持在伪装页链路
* 运势页展示用户输入的 `luckyNumber`
